...
import com.olivierboucher.model.walmart.WMEpicerieProduct;
import com.olivierboucher.model.walmart.WMEpicerieRebate;
...
public class WMEpicerieParser extends EpicerieParser {
    ...
    private void setProductId(WMEpicerieProduct product){
        if(element.select("div[data-sku-sku]").first() != null){
            long id = Long.parseLong(element.select("div[data-sku-sku]").first().attr("data-sku-sku").replaceAll("[^0-9]", ""));
            product.setSku(id);
        }
    }
